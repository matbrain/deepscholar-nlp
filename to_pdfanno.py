    ...
    if file.name.endswith(".pdf"):
        ...
        if len(lines) == 0:
            ...
            for line in lines:
                ...
                page_indexs.append(int(items[0]))
                char = items[1]
                chars.append(char)
                char2pos.append(pos)
                pos += len(char)
                ...
                if delim == "S":
                    ...
                    pos += 1
            text = "".join(chars)
            return {"text": text, "char2pos": char2pos, "anno": [], "bboxes": bboxes, "page_indexs": page_indexs}
    else:
...
def main():
    ...
    for doc_id, doc_json in doc_dict.items():
        ...
        if len(doc_json["anno"]) > 0:
            ...
            page_indexs = doc_json["page_indexs"]
            bboxes = doc_json["bboxes"]
            annos = [[] for _ in range(page_indexs[-1])]
            for anno in doc_json["anno"]:
                s, e = anno[0], anno[1]
                if s >= 0 and e >= 0:
                    bbox = merge_bboxes(bboxes[s:e + 1])
                    page_index = page_indexs[s]
                    if page_index != page_indexs[e]:
                        print("error")
                    # assert page_index == page_indexs[e + 1]
                    annos[page_index - 1].append({"bbox": bbox, "label": anno[2]})
